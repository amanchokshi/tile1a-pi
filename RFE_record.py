...
class obs(object):
    ...
    def record(self,obslen=None):#,data='trialrecord.txt',log='triallog.txt'):
        ...
        #datafile = '%s%s_%s.txt' %(RFE.tiles[self.tile],RFE.pols[self.tile],self.obs_date)
    ...
    def store_data(self):
        ...
        #print 'this tile',RFE.tiles[self.tile],
        #cmd = "mv *%s%s_%s.txt ./data/%s" %(RFE.tiles[self.tile],RFE.pols[self.tile],self.obs_date,self.obs_date)
	phys_addr = RFE.usb_map['ttyUSB%d' %int(self.tile)]
        tile_name = RFE.physical_map[phys_addr]
